...
def get_tsne_df(fnc, dx):
    ...
    tsne_df = pd.DataFrame(data=cal_tsne(cal_pca(get_res_df(fnc, dx))),
                           columns=["D1", "D2"], index=expr_res.index)
    return pd.concat([tsne_df, new_pheno], axis=1)


def plot_tsne(fnc, dx):
    tsne_df = get_tsne_df(fnc, dx)
...
def main():
    for dx in ["MDD", "Schizo"]:
        ...
        save_plot(qq, "shared_clustering_%s" % dx.lower())
