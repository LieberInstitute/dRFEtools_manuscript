...
pca_norm_data <- function(diagnosis){
    ## Load voom normalized data
    load(paste0("../../_m/", tolower(diagnosis), "/voomSVA.RData"))
...
pca_res_data <- function(diagnosis){
    ## Read in residualized data
    fname = paste0("../../_m/", tolower(diagnosis),
...
merge_expr <- function(covars, fnc, diagnosis){
    df <- inner_join(memEST(covars), fnc(diagnosis), by="RNum")
...
fit_model <- function(covars, diagnosis, qsv, norm, identity){
    if(qsv){
        ...
        if(norm){
            est_fit0 <- memEXPR(covars, memNORM, diagnosis) %>%
                group_by(Covariate, PC) %>%
                do(fitEST = broom::tidy(lm(Variable ~ PC_values, data = .)))
        } else {
            est_fit0 <- memEXPR(covars, memRES, diagnosis) %>%
                group_by(Covariate, PC) %>%
                do(fitEST = broom::tidy(lm(Variable ~ PC_values, data = .)))
        }
    }
...
tile_plot <- function(covars, diagnosis, qsv=TRUE, norm=TRUE, identity=TRUE){
    ## Tile plot (heatmap)
    my_breaks <- c(0.05, 0.01, 0.005, 0)
    xlabel = "Covariate"
    if(qsv){
        ylabel = "qSV"; out = "qsv"
        tile_plot0 <- memFIT(covars, diagnosis, qsv, norm, identity) %>%
    ...
    fn = paste0(tolower(diagnosis), "/tilePlot_",out,"_covariates")
...
for(diagnosis in c("MDD", "Schizo", "Bipolar")){
    ...
    dir.create(outdir)
    ## Get covariates
    covarsCont = memPHENO(diagnosis) %>% select(-RNum) %>%
        rownames_to_column("RNum")
    ## Plotting
    tile_plot(covarsCont, diagnosis)
    tile_plot(covarsCont, diagnosis, FALSE, FALSE, TRUE)
    tile_plot(covarsCont, diagnosis, FALSE, TRUE, FALSE)
    tile_plot(covarsCont, diagnosis, FALSE, FALSE, FALSE)
}
