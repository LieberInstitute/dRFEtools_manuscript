"""
Explore variation between control and case.
"""
import numpy as np
import pandas as pd
...
def get_deg(dx):
    ...
    fn = '../../_m/%s/diffExpr_CTLvs%s_full.txt' % (dx.lower(), config[dx])
    deg = pd.read_csv(fn, sep='\t', index_col=0).sort_values('adj.P.Val')
    return deg[(deg['adj.P.Val'] < 0.05)].copy()
...
@lru_cache()
def get_deg_res_df(dx):
    """
    Merge DE and residualized data after selecting # features.
    """
    newList = list(get_deg(dx).sort_values("P.Value").index)
    return get_residualized(dx)[newList]
