...
plot_corr <- function(bigdata){
    dt <- bind_rows(bigdata)
    sca <- ggscatter(dt, x="Rank", y="Partial_R2", #add="reg.line",
                     add.params=list(color="blue", fill="lightgray"),
                     conf.int=TRUE, cor.coef=FALSE, facet.by="Feature",
                     panel.labs.font=list(face="bold"), scales="free_x",
                     ggtheme=theme_pubr(base_size=15, border=TRUE))
    save_plot(sca, "rank_partial_corr", 9, 6)
}
